...
%=====================================================================
function testResult = StressTestLarsonMiller( lmModel, creepData ) 
  ...
  for( c=1:length(testResult.tr) ) 
    for( r=1:length(testResult.T) )
      ...
      pLM = ConvTemp(T,'c','k')*( lmModel.Clm + log10( tr ) );

      
      pLMmc = fliplr( lmModel.masterCurve.coefficients' );
      pLMmc( length(pLMmc) ) = pLMmc( length(pLMmc) ) - pLM;
