...
%=====================================================================
function testResult = StressTestMansonSuccop( msModel, creepData ) 
  ...
  for( c=1:length(testResult.tr) ) 
    for( r=1:length(testResult.T) )
      ...
      pMS = log10( tr ) + msModel.Cms*ConvTemp(T,'c','k');

      pMSmc = fliplr( msModel.masterCurve.coefficients' );
      pMSmc( length(pMSmc) ) = pMSmc( length(pMSmc) ) - pMS;
